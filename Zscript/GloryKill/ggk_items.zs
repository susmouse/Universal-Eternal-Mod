...
class SuperHealthBonus : HealthBonus
{
	...
	Actor plr; // 指向玩家Actor，用于实现自动拾取/追踪效果。
	Default
	{
		Inventory.PickupMessage "Glory kill bonus"; // 拾取时显示的消息。
		Inventory.Amount 1; // 基础回复量 (HealthBonus本身会回复1点，实际回复量在PickupMessage中处理)。
		Scale 0.5; // 缩放比例。
		Radius 2; // 碰撞半径。
		Height 2; // 碰撞高度。
	}
	
	// 覆写 PickupMessage 方法，返回拾取时显示的消息，并在此处实际增加玩家生命值。
	override String PickupMessage()
	{
		// 给予玩家 sv_glorykillhealth 数量的生命值，减1是因为HealthBonus基类会给1点。
		plr.A_SetHealth(plr.health + sv_glorykillhealth-1); 
		return "Glory kill bonus +"..sv_glorykillhealth.." hp"; // 返回格式化的消息。
	}
	
	// 覆写 Tick 方法。
	override void Tick()
	{
		if(pos.z == floorz) bNOGRAVITY = true; // 如果物品在地面上，则禁用重力 (防止陷入地下)。
		livetics++; // 增加存在帧数。
		// 如果玩家 (plr) 存在，与物品有一定距离，且物品已存在超过10帧。
		if(plr && Distance2D(plr) > radius && livetics > 10)
		{
			double spd = 40.; // 飞向玩家的速度。
			// 计算从物品到玩家的向量 (目标是玩家视线高度的一半)。
			vector3 delta = ( plr.pos.x - pos.x, plr.pos.y - pos.y, (plr.pos.z+plr.player.viewheight/2) - pos.z );
			double angleto = AngleTo(plr); // 计算物品指向玩家的水平角度。
			double pitchto = VectorAngle( sqrt(delta.y * delta.y + delta.x * delta.x), delta.z ); // 计算物品指向玩家的垂直角度。
			// 计算新的位置，使其向玩家移动。
			vector3 neworigin = Vec3Offset(cos(angleto) * spd,  sin(angleto) * spd, sin(pitchto) * spd);
			SetOrigin(neworigin,true); // 设置新位置。
		}
		super.Tick(); // 调用父类的Tick。
	}
}